...
#include <my_robot_interfaces/msg/pose_goal.hpp> 
using MoveGroupInterface = moveit::planning_interface::MoveGroupInterface;
using Bool = example_interfaces::msg::Bool;
using PoseGoal = my_robot_interfaces::msg::PoseGoal;
...
public:
    Commander(std::shared_ptr<rclcpp::Node> node)
    {
        ...
        joint_target_sub_ = node_->create_subscription<std::vector<double>>(
            "joint_target", 10, std::bind(&Commander::JointTargetCallback, this, _1)
        );
        pose_goal_sub_=node_->create_subscription<PoseGoal>(
            "pose_goal", 10, std::bind(&Commander::PoseTargetCallback, this, _1)
        );
    }
...
private:
    ...
    void JointTargetCallback(const std::vector<double> &msg)
    {
        goToJointTarget(msg);
    }
    void PoseTargetCallback(const PoseGoal &msg)
    {
    ...
    rclcpp::Subscription<std::vector<double>>::SharedPtr joint_target_sub_;
    rclcpp::Subscription<PoseGoal>::SharedPtr pose_goal_sub_;
};
